...
export const toggleListBtn = (isList) => {
	if (!isList) elements.listMenu.classList.add('hide');
	else elements.listMenu.classList.remove('hide').add('show');
};

export const toggleListMenu = (numList) => {
	elements.listMenu.style.visibility = numList > 0 ? 'visible' : 'hidden';
};

/* const limitIngredient = (ingredient, limit = 35) => {
	const newIngredient = [];
	// console.log('ingredient length:', ingredient.length);
	if (ingredient.length > limit) {
		ingredient.split(' ').reduce((acc, cur, index) => {
			console.log('acc:', acc);
			console.log('cur:', cur);
			console.log('index:', index);
			if (acc + cur.length <= limit) {
				newIngredient.push(cur);
				// return;
				...
				newIngredient.push(splitTitle);
			}
			return acc + cur.length;
		}, 0);
		return `${newIngredient.join(' ')}`;
	}
	return ingredient;
}; */
