...
const controlList = () => {
	...
	// if (!state.recipe.title) listView.renderTitle(state.recipe.title);
	...
	// listView.renderTitle(title.toUpperCase());
	if (!state.list) {
		state.list = new List();

		// console.log('state.recipe:', state.recipe.title);
		// console.log('state.list.id:', state.list.id);
		// Add each ingredient to the list
		/* if (!deleteButton) {
			const length = state.list.items.length;
			listView.deleteBtn(length);
		} */
		state.recipe.ingredients.forEach((el) => {
...
window.addEventListener('load', () => {
	...
	// listView.renderTitle(state.recipe.title);
