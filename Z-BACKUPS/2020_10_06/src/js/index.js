...
// console.log(main.randomImg(proteins));
...
cats.forEach((name) => {
	...
	// renderLoader(document.querySelector('.choose-cat'));
	...
	if (name === 'proteins') {
		// renderLoader(parent);
		cardImg.src = main.randomImg(proteins);
		// clearLoader();
	} else if (name === 'dishes') {
		// renderLoader(parent);
		cardImg.src = main.randomImg(dishes);
		// clearLoader();
	} else if (name === 'fruits') {
		// renderLoader(parent);
		cardImg.src = main.randomImg(fruits);
		// clearLoader();
	} else if (name === 'veggies') {
		// renderLoader(parent);
		cardImg.src = main.randomImg(veggies);
		// clearLoader();
	} else if (name === 'sweets') {
		// renderLoader(parent);
		cardImg.src = main.randomImg(sweets);
		// clearLoader();
	} else if (name === 'misc') {
		// renderLoader(parent);
		cardImg.src = main.randomImg(misc);
		// clearLoader();
	}
	clearLoader();
});
...
const controlRecipe = async () => {
	...
	if (id) {
		...
		try {
			...
			setTimeout(() => {
				recipeView.updateDynamicServings(state.recipe.servings);
			}, 9000);
		} catch (err) {
			console.log(err);
...
window.addEventListener('keypress', (e) => {
	if (e.key === 'Enter' || e.keyCode == '13') clearSearchWarning();
});
